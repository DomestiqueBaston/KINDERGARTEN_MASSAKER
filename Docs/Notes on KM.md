...
- The teacher (see *1 below) running randomly then stops looking around then resumes. When close enough to the Alien and kids involved in a fight, tries to stop the mess (she is an obstacle: running then stopping, trying to calm things down staying between kids and Alien).
...
Technically
	...
	Somewhat random:
		...
		Teacher (Stays where she is then runs somewhere else then resume… Until she detects a short ranged fight between kids and the Alien and tries to take place between kids and the Alien (eventually) forcing a way between colliders – (see *1 above))
	...
	The Teacher has 2 colliders:
		...
		1 fight detection collider (!).

	The Alien has 2 colliders:
