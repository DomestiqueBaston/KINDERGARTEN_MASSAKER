...
func _ready():
	...
	$Tot_in_STYLE.text = " Tot in \n" + str(stepify(Autoload.elapsed_time, 1)) + " sEkundeN!"
	$Tot_in_TEXTURE.text = " Tot in \n" + str(stepify(Autoload.elapsed_time, 1)) + " sEkundeN!"
	$BESTE_STYLE.text = "Beste: " + str(stepify(Autoload.best_time, 1)) + "''"
	$BESTE_TEXTURE.text = "Beste: " + str(stepify(Autoload.best_time, 1)) + "''"
